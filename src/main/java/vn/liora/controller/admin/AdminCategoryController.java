...
public class AdminCategoryController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<CategoryResponse>> addCategory(@Valid @RequestBody CategoryCreationRequest request) {
        ...
        try {
            ...
            if (status != null && !status.isEmpty()) {
                List<Category> filteredCategories = categories.getContent().stream()
                    .filter(category -> {
                        if ("active".equals(status)) return category.getIsActive();
                        if ("inactive".equals(status)) return !category.getIsActive();
                        return true;
                    })
                    .toList();
