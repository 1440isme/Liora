        ...
        const productHtml = `
            <div class="card mb-4 ${isReviewed ? 'border-success' : ''}" data-order-product-id="${product.idOrderProduct}">
                <div class="card-body">
                    ...
                    <!-- Media Upload Section (chỉ hiển thị khi chưa đánh giá) -->
                    ${!isReviewed ? `
                    <div class="mb-3">
                        <label class="form-label fw-medium">Thêm hình ảnh và video (tùy chọn):</label>
                        <div class="media-upload-section">
                            <!-- Image Upload -->
                            <div class="mb-3">
                                <label class="form-label small fw-medium text-muted">Hình ảnh:</label>
                                <div class="image-upload-container">
                                    <div class="image-upload-area" onclick="triggerImageUpload(${product.idOrderProduct})">
                                        <div class="upload-icon">
                                            <i class="fas fa-images fa-2x text-muted"></i>
                                        </div>
                                        <div class="upload-text">
                                            <p class="mb-1">Chọn hình ảnh</p>
                                            <small class="text-muted">JPG, PNG, GIF tối đa 1MB (tối đa 5 ảnh)</small>
                                        </div>
                                        <input type="file"
                                               class="image-input d-none"
                                               id="imageInput_${product.idOrderProduct}"
                                               accept="image/*"
                                               multiple
                                               data-order-product-id="${product.idOrderProduct}">
                                    </div>
                                    <div class="image-preview-container" id="imagePreview_${product.idOrderProduct}">
                            ...
                            <!-- Video Upload -->
                            <div class="mb-3">
                                <label class="form-label small fw-medium text-muted">Video:</label>
                                <div class="video-upload-container">
                                    <div class="video-upload-area" onclick="triggerVideoUpload(${product.idOrderProduct})">
                                        <div class="video-upload-icon">
                                            <i class="fas fa-video fa-2x text-muted"></i>
                                        </div>
                                        <div class="video-upload-text">
                                            <p class="mb-1">Chọn video</p>
                                            <small class="text-muted">MP4, MOV, AVI tối đa 5MB (tối đa 1 video)</small>
                                        </div>
                                        <input type="file"
                                               class="video-input d-none"
                                               id="videoInput_${product.idOrderProduct}"
                                               accept="video/*"
                                               data-order-product-id="${product.idOrderProduct}">
                                    </div>
                                    <div class="video-preview-container" id="videoPreview_${product.idOrderProduct}">
                                        <!-- Preview videos will be added here -->
                                    </div>
                                </div>
                            </div>
                        </div>
...
let uploadedImages = {}; // Store uploaded images by orderProductId
let uploadedVideos = {}; // Store uploaded videos by orderProductId

// Trigger image upload for a specific product
function triggerImageUpload(orderProductId) {
    const input = document.getElementById(`imageInput_${orderProductId}`);
    if (input) {
        input.click();
    }
}

// Trigger video upload for a specific product
function triggerVideoUpload(orderProductId) {
    const input = document.getElementById(`videoInput_${orderProductId}`);
    if (input) {
        input.click();
    }
}
...
function handleImageUpload(event, orderProductId) {
    const files = Array.from(event.target.files);
    const previewContainer = document.getElementById(`imagePreview_${orderProductId}`);
    ...
    const currentImages = uploadedImages[orderProductId] || [];
    if (currentImages.length + files.length > maxFiles) {
        showToast(`Tổng số ảnh không được vượt quá ${maxFiles}`, 'error');
        event.target.value = '';
        return;
    }
    ...
    for (let file of files) {
        ...
        if (file.size > maxSize) {
            showToast(`Kích thước file không được vượt quá 1MB`, 'error');
            event.target.value = '';
            return;
        }
    }
    ...
    uploadImagesToServer(files, orderProductId, previewContainer);
...
async function uploadImagesToServer(files, orderProductId, previewContainer) {
    const token = localStorage.getItem('access_token');
    if (!token) {
        showToast('Vui lòng đăng nhập để tải ảnh', 'error');
        return;
    }

    // Show loading state
    previewContainer.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm" role="status"></div> Đang tải ảnh...</div>';
    ...
    try {
        ...
        const response = await fetch('/api/reviews/media/images', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`
            },
        ...
        if (response.ok && result.success) {
            ...
            if (!uploadedImages[orderProductId]) {
                uploadedImages[orderProductId] = [];
            }
            uploadedImages[orderProductId] = uploadedImages[orderProductId].concat(result.imagePaths);

            // Show preview
            showImagePreview(orderProductId, previewContainer);

            showToast(`Đã tải lên ${files.length} ảnh thành công`, 'success');
        } else {
            throw new Error(result.message || 'Không thể tải ảnh');
        }
    } catch (error) {
        console.error('Error uploading images:', error);
        previewContainer.innerHTML = '<div class="image-upload-error">Lỗi khi tải ảnh. Vui lòng thử lại.</div>';
        showToast('Lỗi khi tải ảnh: ' + error.message, 'error');
    }
}

// Show image preview
function showImagePreview(orderProductId, previewContainer) {
    const images = uploadedImages[orderProductId] || [];
    ...
    let previewHtml = '<div class="d-flex flex-wrap">';
    images.forEach((imageUrl, index) => {
        previewHtml += `
            <div class="image-preview-item">
                <img src="${imageUrl}" alt="Preview ${index + 1}" loading="lazy">
                <button type="button" class="image-remove-btn" onclick="removeImage(${orderProductId}, ${index})" title="Xóa ảnh">
                    ×
                </button>
            </div>
        `;
...
function removeImage(orderProductId, imageIndex) {
    if (uploadedImages[orderProductId] && uploadedImages[orderProductId][imageIndex]) {
        uploadedImages[orderProductId].splice(imageIndex, 1);
...
// Update submitAllReviews to include uploaded images
async function submitAllReviews() {
    const token = localStorage.getItem('access_token');
    if (!token) {
        alert('Vui lòng đăng nhập để đánh giá');
    ...
    $('.card[data-order-product-id]').each(function () {
        ...
        if (pendingChanges) {
            ...
            updatedReviews.push({
                ...
                content: pendingChanges.content || '',
                anonymous: pendingChanges.anonymous,
                imagePaths: uploadedImages[orderProductId] || []
            });
            hasValidReview = true;
            return;
        }
...
// Add event listeners for image inputs when modal is shown
$(document).on('shown.bs.modal', '#reviewModal', function () {
    // Add change event listeners to all image inputs
    $('input[type="file"][id^="imageInput_"]').each(function() {
        $(this).off('change').on('change', function(event) {
            const orderProductId = $(this).data('order-product-id');
            handleImageUpload(event, orderProductId);
        });
    });
});
